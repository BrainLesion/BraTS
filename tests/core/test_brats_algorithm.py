...
class TestBraTSAlgorithm(unittest.TestCase):
    ...
    def test_infer_single(self,mock_inference_setup,mock_input_sanity_check, mock_run_container):
        
        # Mock InferenceSetup context manager
        mock_inference_setup_ret = mock_inference_setup.return_value
        mock_inference_setup_ret.__enter__.return_value = (self.data_folder, self.output_folder)
        
        
        def create_output_file(*args, **kwargs):
            subject_id = self.segmenter.algorithm.run_args.input_name_schema.format(id=0)
            alg_output_file = self.output_folder / OUTPUT_NAME_SCHEMA[self.segmenter.task].format(subject_id=subject_id)
    ...
    @patch("brats.core.segmentation_algorithms.input_sanity_check")
    @patch("brats.core.brats_algorithm.InferenceSetup")
    def test_infer_batch(self,mock_inference_setup,mock_input_sanity_check, mock_run_container):
