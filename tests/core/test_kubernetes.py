...
def test_build_command_args_with_additional_and_params(dummy_algorithm, monkeypatch, tmp_path):
    algo = dummy_algorithm(year=2024, with_additional=True)
    monkeypatch.setattr(k8s, "_get_parameters_arg", lambda algorithm: " --params=/mlcube_io3/foo.json")
...
def test_download_additional_files_success(dummy_algorithm, monkeypatch):
    algo = dummy_algorithm(year=2024, with_additional=True)
    monkeypatch.setattr(
        k8s, "_get_zenodo_metadata_and_archive_url", lambda record_id: ({"version": "2"}, "http://example/archive.zip")
    )
    exec_calls = []
    ...
    p = k8s._download_additional_files(algorithm=algo, pod_name="p", namespace="ns", mount_path="/data")
...
def test_download_additional_files_zenodo_failure(dummy_algorithm, monkeypatch):
    algo = dummy_algorithm(year=2024, with_additional=True)
    monkeypatch.setattr(k8s, "_get_zenodo_metadata_and_archive_url", lambda record_id: None)
    with pytest.raises(k8s.ZenodoException):
        k8s._download_additional_files(algorithm=algo, pod_name="p", namespace="ns", mount_path="/data")
...
def test_check_files_in_pod_uploads_missing(monkeypatch, tmp_tree):
    ...
    monkeypatch.setattr(k8s, "_execute_command_in_pod", lambda **kw: "No such file or directory")
...
def test_upload_files_to_pod(monkeypatch, tmp_path):
    ...
    class FakeResp:
        ...
        def close(self):
            pass
    monkeypatch.setattr(
        k8s, "_execute_command_in_pod", lambda **kw: FakeResp()
    )
...
def test_create_pvc_skips_if_exists(monkeypatch):
    ...
    mock_core.list_namespaced_persistent_volume_claim.return_value = types.SimpleNamespace(items=[pvc])
    monkeypatch.setattr(k8s.client, "CoreV1Api", lambda: mock_core)
...
def test_create_finalizer_job(monkeypatch):
    ...
    name = k8s._create_finalizer_job(job_name="job-final", namespace="ns", pvc_name="pvc", mount_path="/data")
...
def test_create_namespaced_job_deletes_old_pod_and_creates_new(monkeypatch):
    ...
    mock_batch.list_namespaced_job.return_value = types.SimpleNamespace(items=[types.SimpleNamespace(metadata=types.SimpleNamespace(name="job-1"))])
...
def test_run_job_year_2024_flow(monkeypatch, tmp_tree, tmp_path, dummy_algorithm):
    ...
    monkeypatch.setattr(k8s, "_get_zenodo_metadata_and_archive_url", lambda record_id: ({"version": "1"}, "http://example/zip"))
    ...
    def fake_create_job(**kwargs):
        return "job-pod"
    monkeypatch.setattr(k8s, "_create_namespaced_job", lambda **kw: fake_create_job(**kw))

    # CoreV1Api read_namespaced_pod progression
    mock_core = MagicMock()
    # First loop waits until init container running
    mock_core.read_namespaced_pod.return_value = _mk_pod(name="job-pod", phase="Running", init_running=True)
    monkeypatch.setattr(k8s.client, "CoreV1Api", lambda: mock_core)
    ...
    monkeypatch.setattr(k8s, "_download_additional_files", lambda **kw: Path("/data/12345_v1"))
    ...
    mock_core.read_namespaced_pod.side_effect = [
        ...
        _mk_pod(name="job-pod", phase="Succeeded", init_running=True),  # completion loop
    ]
...
def test_run_job_year_2025_flow(monkeypatch, tmp_tree, tmp_path, dummy_algorithm):
    ...
    monkeypatch.setattr(k8s, "_create_namespaced_pvc", lambda **kw: fake_create_pvc(**kw))
